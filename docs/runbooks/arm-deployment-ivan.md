...
> ⚠️ **BL-53 — кэш `.env` и `configs/*.yaml`.** После изменения `.env` или любого файла в `configs/*.yaml` нужен `Ctrl+C` → `streamlit run src/ui/app.py` заново → `Ctrl+Shift+R` в браузере. Кнопка `Rerun` в правом верхнем углу Streamlit перезапускает только Python-код страницы, но **не** выполняет `load_dotenv()` повторно и **не** перечитывает YAML-конфиги — они остаются в памяти процесса с момента старта. Hot-reload Streamlit срабатывает только на изменения в `src/`. Без полного рестарта процесса вы будете 5–10 минут гадать «почему правка `.env` не применилась» (см. отчёт тестировщика §1.6 / Проблема #4).
