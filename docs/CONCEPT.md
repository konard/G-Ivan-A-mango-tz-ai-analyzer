...
| Поле | Значение |
|------|----------|
| **Описание** | Сохранение результатов анализа в исходный формат файла с добавлением колонок результата. **Минимальный MVP-набор колонок:** `[Статус]`, `[Комментарий]`, `[Confidence]`, `[RunID]`. **Расширенная схема** (`[Рекомендация]`, `[Требует ревью]`, `[Провайдер]`, `[Ошибка]`, дополнительные диагностические поля) выносится в **ADR-002** и принимается после Пилота на основе обратной связи БА. |
...
| **Критерий приёмки** | Выходной файл сохраняет структуру исходника (порядок строк, заголовки, оригинальные данные); все четыре MVP-колонки заполнены для каждой строки; кодировка UTF-8; файл открывается в MS Excel / LibreOffice без предупреждений. |
| **Артефакты** | `src/exporters/excel_exporter.py`, `tests/test_pipeline.py::test_run_analysis_end_to_end` |
| **Связанные решения** | Расширение схемы экспорта — **ADR-002 (пост-пилот)**; пороги `Confidence` — `configs/classification_rules.json` (`min_confidence_for_auto: 0.85`). |
...
|---|--------|:------:|------------------|--------|
| 1 | Расширение схемы экспорта | ✅ Закрыт | MVP — минимальный набор `[Статус]`, `[Комментарий]`, `[Confidence]`, `[RunID]`. Расширенная схема (`[Рекомендация]`, `[Требует ревью]`, `[Провайдер]`, `[Ошибка]`, …) выносится в **ADR-002 после Пилота** на основе обратной связи БА. | FR-06, раздел 4 |
