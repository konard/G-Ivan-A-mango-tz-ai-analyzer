...
**Версия:** 2.0 | **Дата:** 2026-05-15 | **Статус:** Approved for MVP
...
**Связанная задача:** [issue #37](https://github.com/G-Ivan-A/mango-tz-ai-analyzer/issues/37)
...
| Поле | Значение |
|------|----------|
| **Описание** | Сохранение результатов анализа в исходный формат файла с добавлением колонок результата. **Минимальный обязательный набор:** `[Статус]`, `[Комментарий]`, `[Цитаты]`, `[Confidence]`. **Расширенный набор (используется в текущей реализации):** дополнительно `[Уверенность]`, `[Рекомендация]`, `[Требует ревью]`, `[Провайдер]`, `[Ошибка]`, `[RunID]` — для аудируемости (зафиксируется в ADR-002 при первой ревизии). |
| **Вход** | Исходный файл ТЗ + список `ClassificationResult` |
| **Выход** | Файл в исходном формате с добавленными колонками |
| **Критерий приёмки** | Выходной файл сохраняет структуру исходника (порядок строк, заголовки, оригинальные данные); новые колонки заполнены для каждой строки; кодировка UTF-8; файл открывается в MS Excel / LibreOffice без предупреждений. |
| **Артефакты** | `src/exporters/excel_exporter.py`, `tests/test_pipeline.py::test_run_analysis_end_to_end` |

### FR-07. Streamlit UI
| Поле | Значение |
|------|----------|
| **Описание** | Web-интерфейс с двумя вкладками: **«Анализ ТЗ»** (загрузка файла, запуск анализа с индикатором прогресса, скачивание результата) и **«Концепция и БЗ»** (рендеринг `docs/CONCEPT.md`, ссылки на GitHub Issues и реестр источников). |
| **Вход** | Файл ТЗ от пользователя |
| **Выход** | Файл с результатами анализа |
| **Критерий приёмки** | Приложение запускается командой `streamlit run src/app.py`; обработка файла достижима **≤ 3 кликами** (upload → run → download); вкладка «Концепция» корректно рендерит Markdown, внутренние ссылки кликабельны; кнопка «Запустить анализ» вызывает реальный `src.pipeline.run_analysis`, а не stub. |
| **Артефакты** | `src/app.py`, `docs/screenshots/ui-tab-analysis.png`, `docs/screenshots/ui-tab-concept.png` |
...
[Файл .xlsx/.docx]
   ...
   → FR-06 Экспорт в исходный формат
   → FR-08 JSON-лог с run_id
```
...
Открытые вопросы фиксируются здесь и решаются Product Owner. Полный список с обоснованиями — в [`docs/analysis/2026-05-15_analysis_repo-state-and-mvp-recommendations_v1.md`](analysis/2026-05-15_analysis_repo-state-and-mvp-recommendations_v1.md), раздел 7.

1. **Расширение схемы экспорта.** Зафиксировать расширенный набор колонок в этом документе или вынести в отдельный ADR-002.
2. **`.docx`-парсинг/экспорт.** Остаётся Out-of-Scope MVP или поднимается до MUST.
3. **Параллелизация LLM-вызовов.** Допустимы ли одновременные запросы к одному провайдеру (зависит от TOS GigaChat / YandexGPT).
4. **Маскирование ФИО.** Включить в `configs/masking_rules.yaml` или отложить до Пилота.
5. **Human-in-the-Loop UX.** Уровень редактирования: read-only review / inline edit / save back.
6. **Source of Truth для KB-источников.** Общий диск / SharePoint / приватный репозиторий.
7. **Stub-провайдер в production.** Допустимо ли отвечать `НД, confidence=0.0` при недоступности всех 4 провайдеров.
