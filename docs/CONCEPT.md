...
Для KB-тестового UI (`src/ui/app.py`) поддержан экспериментальный
**Multi-hop Retrieval**: при `rag.multi_hop_enabled: true` он запускается
только в режиме «Консультация» и ограничен `rag.max_hops`. Режим «Анализ ТЗ»
жёстко остаётся на one-shot retrieval независимо от флага, чтобы не увеличивать
latency и token budget batch-прогона. Reflection-шаг использует
`prompts/system_rag_reflection_v1.0.md` и строгий JSON
`{"sufficient": boolean, "follow_up": string | null, "confidence": float}`;
timeout, сетевой сбой или невалидный JSON не прерывают UI: поиск деградирует
к последнему накопленному контексту и переходит к финальной генерации ответа.
