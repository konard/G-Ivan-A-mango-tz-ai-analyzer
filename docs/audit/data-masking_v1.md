...
- [ ] Regex-паттерны добавлены в `configs/masking_rules.yaml`.
- [ ] Модуль `src/llm/masking.py` реализован.
- [ ] Юнит-тесты написаны (`tests/test_masking.py`).
- [ ] Интеграция в LLM-пайплайн (между подготовкой промпта и вызовом провайдера) завершена.
- [ ] Аудит логов на утечки добавлен в чек-лист релиза.

## 6. Open Questions
- Какой формат токенов-заглушек считать каноническим (`[EMAIL]`, `<EMAIL>`, `__EMAIL__`)? — закрепить в `configs/masking_rules.yaml`.
