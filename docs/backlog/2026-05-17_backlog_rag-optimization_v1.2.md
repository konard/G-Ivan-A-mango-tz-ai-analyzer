# 📦 Бэклог оптимизации RAG-пайплайна (P0–P2) — v1.2
...
- **Дата:** 2026-05-17
- **Версия:** v1.2
- **Предыдущая версия:** [`docs/backlog/2026-05-17_backlog_rag-optimization_v1.md`](2026-05-17_backlog_rag-optimization_v1.md) (содержит v1 и v1.1).
- **Автор:** konard (AI issue solver, по [issue #83](https://github.com/G-Ivan-A/clarify-engine-ai/issues/83))
- **Статус:** Draft → Review
...
- **Связанные документы:**
  ...
  - [`docs/ADR/003-multi-agent-orchestration-draft.md`](../ADR/003-multi-agent-orchestration-draft.md) §4 (Границы / Security boundary), §5 (Negative / R-09 prompt-injection) — стратегический контекст и поверхность атаки.
  ...
  - [`docs/standards/naming-convention.md`](../standards/naming-convention.md) v1.1 — стандарт именования (тип `backlog`, версия `v1.2`).
  - [`docs/standards/export-markup.md`](../standards/export-markup.md) — единая схема разметки результата (§12 / BL-19).
...
- **Связанные Issues:** [#76](https://github.com/G-Ivan-A/clarify-engine-ai/issues/76), [#77](https://github.com/G-Ivan-A/clarify-engine-ai/issues/77), [#79](https://github.com/G-Ivan-A/clarify-engine-ai/issues/79), [#83](https://github.com/G-Ivan-A/clarify-engine-ai/issues/83).

### Терминология (договорённость PO, действует с v1.2)
...
- **Чанкинг** — целевая схема `chunk_size = 512`, `overlap = 64` c guardrails `min=384`, `max=768` (см. [`docs/standards/embedding-model.md`](../standards/embedding-model.md), [`configs/embedding_config.yaml`](../../configs/embedding_config.yaml), backlog BL-06, BL-16b). Старое окно «200–300 / 50» сохраняется в [CONCEPT §6.2](../CONCEPT.md) до синхронизации BL-16b — это **признанный временный рассинхрон**, явно зафиксированный в §8 настоящего документа.

---

## 0. Validation Report (по [issue #83](https://github.com/G-Ivan-A/clarify-engine-ai/issues/83))

> Раздел добавлен в v1.2. Подтверждает закрытие всех 8 пунктов чек-листа из issue #83 и фиксирует diff относительно v1.1.

### 0.1. Закрытие чек-листа валидации (8/8)

| # | Проблема v1.1 | Действие в v1.2 | Где в документе |
|---|----------------|------------------|------------------|
...
| 2 | BL-16 меняет стандарты раньше, чем BL-06 меняет код | BL-16 расщеплён на **BL-16a** (P0: метаданные + STRICT_MODE + маскирование, обновление CONCEPT §5/§7 и `embedding-model.md` §5 в части flags) и **BL-16b** (P1: синхронизация `chunk_size=512`, `overlap=64`, guardrails — выходит **после** BL-06) | §3 (BL-16a), §4 (BL-16b) |
...
| 7 | Трассировка к ADR-003 §4 (Security) | §8 содержит явную строку «ADR-003 §4 — Security boundary / Non-scope»; BL-23 ссылается на ADR-003 §4 и §5 (R-09) как на собственный архитектурный контекст | §8 |
...
| Добавление | — | `BL-16b` | Часть P1: синхронизация `chunk_size = 512`, `overlap = 64`, guardrails в `CONCEPT §6.2`, `embedding-model.md` §5, `embedding_config.yaml` (закрывает чек-лист #2, часть 2). |
...
| Добавление | — | `BL-23` | Log sanitization: маскирование чувствительных данных в JSON-логах FR-08 + редактирование PII в `rag_eval`-отчётах (чек-лист #6, ADR-003 §4/§5). |
...
> **Сквозная нумерация (V-10) сохранена.** Следующий свободный ID после v1.2 — **BL-25**.
...
BL-16a ──┐
         ├─► BL-02 ──┬─► BL-06 ──► BL-16b
         │           │             └─► (синхронизация CONCEPT §6.2 + embedding-model.md §5
         │           │                  + embedding_config.yaml, выполняется ПОСЛЕ кода BL-06)
         │           ├─► BL-09 ──► BL-09.1 (S3 / static-serve, Pilot)
         │           └─► BL-10 (Parent Retrieval, требует и BL-02, и BL-06)
         │
BL-03 ───┤  (STRICT_MODE — не зависит от reindex, может стартовать параллельно P0)
BL-04 ───┤  (маскирование RAG-контекста — независимая правка LLMClient)
BL-22 ───┤  (temperature lock — изолированная правка llm_config.yaml + LLMClient defaults)
BL-23 ───┤  (log sanitization — изолированная правка logger; зависит от FR-08, ADR-003 §4)
BL-05 ───┤  (Golden Set + evaluate_rag.py — не зависит от reindex, нужен для приёмки P0)
   └── BL-05.1 (CI smoke job ≤ 2 мин на 5-Q подвыборке + stub-LLM)

BL-01 ────► (Hybrid в production-пути UI — независим от метаданных, но валидируется BL-05/BL-05.1)
BL-07 ────► (диалоговая память UI — независимая правка UI/session_state)
BL-08 ────► (prompt library — изолированная переразметка prompts/)

BL-11 ────► BL-12 (multi-hop требует подключённого Hybrid и stable Golden Set; включается под флагом)
BL-12 ────► (Query Expansion — параллельно с BL-11; фьюж через RRF)
BL-13 ────► BL-14 (Canonical Cache / Offline Dependency Graph — отдельный ADR; зависят от BL-05, BL-02)
BL-15 ────► (Ollama-tuning — независимо, ускоряет BL-05/BL-05.1)
BL-24 ────► BL-05 (faithfulness gate подключается к evaluate_rag.py из BL-05)
```
...
- **ADR-003 §4 (Security boundary / Non-scope):** §8 содержит строку с явной привязкой BL-23 → ADR-003 §4.

### 0.5. Дифф-сводка относительно v1.1

| Раздел документа | До (v1.1) | После (v1.2) |
|---|---|---|
...
| §4 P1 | BL-06, BL-07, BL-08, BL-09, BL-10 | BL-06 (с `depends_on`), BL-07, BL-08, BL-09.1 (бывший BL-09 «S3»), BL-10 (с `depends_on`), **BL-16b** |
| §5 P2 | BL-11, BL-12, BL-13, BL-14, BL-15 | + **BL-24** (faithfulness gate); явная отметка Neighbour expansion (`#14` анализа) как Out-of-Scope MVP |
...
| §8 Связанная документация | Без ADR-003 §4 | + строка «ADR-003 §4 (Security boundary)» с привязкой BL-23; + строка `configs/llm_config.yaml` (BL-22, BL-23); + явный flag о временном рассинхроне CONCEPT §6.2 vs BL-06 до закрытия BL-16b |
...
| §12 (бывший §12 #79) | Не менялся по сути | Сохранён без изменений: BL-18..BL-21 + V-01..V-10 (нумерация сохранена согласно V-10) |
| §11 (бывший «История изменений») | — | Переименован в §13 «История изменений», добавлена строка v1.2 |
...
`docs/backlog/2026-05-17_backlog_rag-optimization_v1.2.md` (см.
...
naming-convention.md); MINOR-версия `v1.2` отражает уточнения по
issue #83 без изменения выводов.
...
|------------------|-------------|-----------|
| §2.2 Фрагментация контекста | BL-06 (chunker L1), BL-10 (Parent L2), BL-16b (синхронизация стандартов) | Проблема P1 |
...
| **BL-23** | Log sanitization (FR-08 + RAG-eval отчёты) | — | P0 | S (1 д) | Чек-лист валидации #6/#7; [CONCEPT §7 R-03](../CONCEPT.md#7-управление-рисками), NFR-05 (0 утечек); [ADR-003 §4](../ADR/003-multi-agent-orchestration-draft.md#4-границы-и-явный-non-scope) (Security boundary) и §5 Negative (R-09 prompt-injection) — surface attack для логов | JSON-логи FR-08 и отчёты `evaluate_rag.py` сохраняют **исходные** тексты требований и чанков → потенциальная утечка PII / commercial-sensitive данных через файловую систему и build-artifacts CI | (1) Расширить `src/llm/masking.py` функцией `sanitize_log_record(record: dict) -> dict`, применяющей regex-маскирование к полям `payload`, `context`, `answer`, `question`, `chunks[*].text`. (2) Подключить sanitizer как Python-`logging.Filter` в `src/pipeline.py` (`run_id`-aware). (3) Покрыть `tests/test_masking.py::test_log_sanitization_applies_to_evaluate_rag_report`. (4) Обновить [`docs/audit/data-masking_v1.md`](../audit/data-masking_v1.md) разделом «Log sanitization (FR-08 + RAG-eval)» и зафиксировать ссылку на ADR-003 §4 | 0 совпадений regex чувствительных данных в `reports/rag-*.json` и в логах CI; тест `test_log_sanitization_applies_to_evaluate_rag_report` зелёный |
...
| Sprint 2 (1 нед) | BL-06 → BL-16b, BL-07, BL-08, BL-09.1, BL-10 | L1+L2 chunker (с обновлёнными стандартами), диалог, prompt library, Pilot-режим кликабельных цитат |
...
- Требует переоценки рисков (R-02 «Устаревание KB», R-09 «Prompt-injection из KB», см. [ADR-003 §5](../ADR/003-multi-agent-orchestration-draft.md#5-consequences-если-решение-будет-принято) Negative).
...
|---------|---------------------|----------------|----------|
| Cache hit rate | 0 % | ≥ 30 % на корпусе Корпуса требований | BL-13 PoC |
...
|------|--------------|-------------|--------------|
| [`docs/CONCEPT.md`](../CONCEPT.md) §6.2 «Компоненты», п. 2 | Параметры чанкинга `200–300 / 50` → `512 / 64` с диапазоном `[384, 768]` | Секционная структура SaaS-мануалов ([`RAG_OPTIMIZATION_ANALYSIS.md` §3.1](../RAG_OPTIMIZATION_ANALYSIS.md#31-уровень-1--базовая-оптимизация-should--1–2-дня)); 512 — sweet spot bge-m3. **Временный рассинхрон CONCEPT §6.2 vs `embedding_config.yaml` сохраняется до выполнения BL-16b и закрывается в Sprint 2** | BL-06, BL-16b |
...
| [`docs/ADR/003-multi-agent-orchestration-draft.md`](../ADR/003-multi-agent-orchestration-draft.md) §4 «Границы / Non-scope» (Security boundary) | Сохранить трассировку: log sanitization (BL-23) **в scope MVP**, выход за границу offline-агентов (R-09 prompt-injection из KB) **остаётся** в ADR-003 §4/§5 как non-scope MVP | Чек-лист валидации #7: явная ссылка §8 → ADR-003 §4 (Security) | BL-23 |
...
| [`docs/audit/data-masking_v1.md`](../audit/data-masking_v1.md) | Добавить раздел «Log sanitization (FR-08 + RAG-eval)» с явной ссылкой на [ADR-003 §4 (Security boundary)](../ADR/003-multi-agent-orchestration-draft.md#4-границы-и-явный-non-scope) и BL-23 | Контролируемое продвижение лог-санитайзера в production audit; чек-лист валидации #6/#7 | BL-23 |
...
## 9. ✅ Критерии приёмки (Definition of Done) — v1.2

- [ ] Файл `docs/backlog/2026-05-17_backlog_rag-optimization_v1.2.md` создан и соответствует [`naming-convention.md`](../standards/naming-convention.md) v1.1 (тип `backlog`, MINOR-версия `v1.2`).
- [ ] Бэклог содержит **все** задачи P0–P2 (включая §12 BL-18..BL-21) без дубликатов и внутренних противоречий (см. §§3–5, §12).
...
- [ ] §8 содержит явную строку с ссылкой на [ADR-003 §4 (Security boundary)](../ADR/003-multi-agent-orchestration-draft.md#4-границы-и-явный-non-scope) и привязкой к BL-23 (чек-лист валидации #7).
...
- [ ] Раздел §8 явно перечисляет файлы, требующие синхронизации, с обоснованием расхождений (включая временный рассинхрон CONCEPT §6.2 ↔ BL-16b до Sprint 2).
...
- Принятый log sanitizer (BL-23) на стороне FR-08 и evaluate_rag-отчётов как
  обязательный pre-condition для offline-агентов (Security boundary ADR-003 §4/§5).
...
≥ 1 явную привязку к BL-задаче (в т. ч. через §12 BL-18..BL-21).
...
| **FR-02** | Индексация базы знаний | BL-02 (метаданные), BL-06 (chunker L1), BL-10 (Parent), BL-14 (offline deps), BL-16a/BL-16b (стандарты) | KB-схема и reindex |
...
| **FR-06** | Экспорт результатов (multi-format) | BL-19 (стандарт разметки), BL-20 (ExportRouter + адаптеры) | §12 |
| **FR-07** | Streamlit UI | BL-07 (память диалога), BL-09 (базовые цитаты), BL-09.1 (Pilot-режим цитат), BL-21 (UI-селекторы экспорта) | UX |
...
| **NFR-07** | Актуальность KB | ≤ 24 ч | BL-02 (обогащение schema), BL-13 (cache invalidate) | `source_registry.csv` |
| **NFR-08** | Доступность сервиса | ≥ 99 % (Пилот) | BL-15 (ускорение), BL-22 (стабильность) | На Пилоте |
| **NFR-09** | Безопасность загрузки | Лимит 10 МБ | BL-21 (UI-валидация), V-04..V-07 (см. §12) | Multi-sheet `.xlsx` и legacy `.doc` |
...
> Сквозная нумерация BL-18..BL-21 сохранена (V-10). Следующий свободный ID
> после v1.2 — **BL-25**.
...
|---|-----------|-------------------------|----------|------|--------------------------|----------|
| V-01 | Политика данных | `docx_parser.py` плоско склеивает параграфы и ячейки → утрата локаторов | `src/parsers/docx_parser.py`, [`analysis §2.2`](../analysis/2026-05-17_analysis_tz-structure_samples.md#22-распознанные-структурные-паттерны) | Утрата трассируемости (NFR-06) | Поле `locator` (BL-18) + маркер `[Ref:]` (BL-19) | BL-18, BL-19 |
| V-02 | Политика данных | `append_to_original` нарушает CONCEPT §2.3, если включён по умолчанию | CONCEPT §2.3, issue #79 | Нарушение SHA-256 версионирования KB | `output_mode = create_new` — default; `append_mode` под флагом и не в production | BL-19 §5, BL-21 |
| V-03 | PII / Маскирование | До релиза `masking_rules.yaml` v2 файл-отчёт не должен содержать копию содержимого исходника | CONCEPT §10 п.4, [`analysis §2.5`](../analysis/2026-05-17_analysis_tz-structure_samples.md#25-что-не-изучалось-граница-анализа) | Утечка PII через файл-отчёт | В `export-markup.md` §4.2/§4.3 зафиксировано: только `Ref`, не текст | BL-19, FR-05 |
...
| V-08 | Логи / `RunID` | Многократный экспорт одного `run_id` должен оставаться идемпотентным | CONCEPT §4 FR-08, §6.7.3 | Расхождение `xlsx`/`docx`/`md`-отчётов | Один источник результатов в памяти; тест `tests/test_export_router.py` | BL-20 |
| V-09 | Совместимость с ADR-002 | Расширенная схема экспорта (ADR-002) и multi-format export ортогональны | [`ADR-002`](../ADR/002-export-schema-extension.md) | Post-pilot колонки «дрейфуют» | ADR-002: 4 MVP-колонки FR-06 стабильный контракт; новые поля через `schema_version` | BL-19, обновление ADR-002 |
| V-10 | Backlog-гигиена | Нумерация задач сквозная; BL-21 — последний из §12 v1.1 | этот §12 | Дубликаты BL-ID при параллельной правке | BL-18..BL-21 — scope shift `.docx`+multi-format; следующая свободная — **BL-25** (с учётом BL-22..BL-24 из v1.2) | этот §12 + §0 v1.2 |

### 12.3. Сводка изменений документации, инициированных §12

| Файл | Что обновить | Связанный BL |
|------|--------------|---------------|
| [`docs/CONCEPT.md`](../CONCEPT.md) §2.3, §4 FR-01/FR-06/FR-07, §8.1.1, §10 п.2 | Scope shift `.docx`+multi-format export → MVP | BL-18..BL-21 |
| [`docs/ADR/002-export-schema-extension.md`](../ADR/002-export-schema-extension.md) | Note: 4 MVP-колонки сохраняются; multi-format через `schema_version` (V-09) | BL-19, BL-20 |
...
| **v1.2** | **2026-05-17** | **Корректировка валидации по [issue #83](https://github.com/G-Ivan-A/clarify-engine-ai/issues/83):** (1) §0 Validation Report (8/8 чек-листа); (2) BL-16 расщеплён на BL-16a (P0) и BL-16b (P1); (3) BL-10 получил `depends_on: [BL-02, BL-06]` + schema-check; (4) добавлен BL-05.1 (CI smoke-job ≤ 2 мин); (5) BL-09 перенесён в P0 (базовый рендер `file://` + `#page=`), BL-09.1 — в P1 (S3 / static-serve); (6) добавлены BL-22 (temperature lock), BL-23 (log sanitization), BL-24 (faithfulness gate); (7) §8 ссылается на [ADR-003 §4 (Security boundary)](../ADR/003-multi-agent-orchestration-draft.md#4-границы-и-явный-non-scope) с привязкой BL-23; (8) §11 «Матрица покрытия FR/НФТ ↔ BL»; (9) у каждой задачи явные `depends_on`, `priority`, `effort`; (10) терминология «Корпус требований». Документ остаётся `Draft → Review`, кодовых изменений нет. |
