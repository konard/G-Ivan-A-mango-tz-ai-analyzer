...
BL-23 ───┤  (log sanitization — изолированная правка logger; зависит от FR-08, ADR-003 §4.3)
...
| §8 Связанная документация | Без ADR-003 §4 | + строка «ADR-003 v1.1 §4 (Security & Compliance)» с привязкой BL-23 → §4.3 (Log sanitization, `sanitize_for_log()`); + строка `configs/llm_config.yaml` (BL-22, BL-23); + явный flag о временном рассинхроне CONCEPT §6.2 vs BL-06 до закрытия BL-16b |
