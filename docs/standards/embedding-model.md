...
- Согласуется с пунктом 4 концепции о чанкинге 512 токенов, overlap 64, guardrails `[384, 768]` и реестре источников.
...
Кодовые константы, конфиг и документация используют один контракт:
...
`metadata`. Минимальный пользовательский контракт для цитирования и BL-10:
`page_number`, `section_title`, `section_number`, `product`, `parent_id`.
Поле `source` и `chunk_idx` уже присутствуют в MVP; поля секций добавляются
задачей BL-02 в окне «Reindex & Metadata Enrichment».
