...
## ⏱ Первый ответ на CPU-only АРМ — 60–90 сек

**Что вы увидите.** При самом первом запросе в новой сессии (или после
перезагрузки рабочего места) спиннер задержится надолго:

> ⏱ Спрашиваем LLM (GigaChat → OpenRouter → Ollama)…
> Первый ответ на CPU может занять 60–90 сек.

**Это нормально, система не зависла.** На CPU-only АРМ локальная Ollama
поднимает модель `qwen2.5:7b` в память при первом обращении — это разовая
плата. Последующие запросы в той же сессии возвращают ответ за **5–15 сек**.

**Если хотите ускорить первый запрос** — нажмите в сайдбаре кнопку
**«🔥 Прогреть модель»** *до* первого вопроса. Кнопка отправляет в Ollama
короткий фоновый пинг и кладёт модель в кэш на 10 минут. Кнопка видна
только когда:

- администратор включил `ui.debug_mode: true` в `configs/ui_config.yaml`;
- ИЛИ `OLLAMA_BASE_URL` указывает на `localhost` / `127.0.0.1` (стандартный
  локальный сценарий пилотного АРМ).

На облачном `OLLAMA_BASE_URL` кнопка скрыта намеренно — мы не флудим
прогревом чужой сервис.

> 📘 См. также раздел «Долгий первый ответ» в
> [4. Устранение проблем](04_troubleshooting.md), если кнопка не помогла.
